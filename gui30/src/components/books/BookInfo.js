...
class BookInfo extends Component {
    ...
    componentDidMount() {
        ...
        fetchHandler(this.apiPath + '/' + topicId, null,(json) => {
            this.loadComments(json._links.comments.href);

           // console.log(topicId);
            json.id = topicId;

            this.setState({loading: false, book: json, commentLink:json._links.comments.href })
    ...
    loadComments(link) {
        fetchHandler(link, null,(json) => {
            ...
            var objects = json._embedded.comments;
            ...
            this.setState({comments: json._embedded.comments })
        }, (json) => this.setState({loading: false, errorMsg: json}))
    }

    saveComment(comment) {
        fetchHandler('/datarest/comments', {
            method: "post",
            headers: {
